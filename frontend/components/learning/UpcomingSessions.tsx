      ...
      {sessions.map((s) => (
        <div key={s.id}
          ...
          <div className="flex-1 min-w-0">
            <p className="text-gray-900 dark:text-white text-sm font-medium truncate">{s.resource_title ?? s.title}</p>
            <p className="text-gray-500 text-xs mt-0.5">{formatDate(s.scheduled_at)} · {s.duration_minutes}min</p>
          </div>
          {s.calendar_event_id ? (
