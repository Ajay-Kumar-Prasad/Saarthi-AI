...
function ReviewTab({
  cards, setCards, loaded, setLoaded, autoLoad, onAutoLoaded,
}: {
  cards: Card[]
  ...
  // Auto-load when triggered from Create tab
  useEffect(() => {
    if (autoLoad) {
      load()
      onAutoLoaded?.()
  ...
  if (done) return (
    <div className="text-center py-4">
      ...
      <button
        ...
        className="px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-white text-sm rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
      >
        Review again
      </button>
    </div>
  ...
  if (!loaded || cards.length === 0) return (
    <div>
      ...
      >
        {loading ? "Loading cards…" : loaded ? "No cards due — refresh?" : "Load due cards"}
      </button>
    </div>
  )
...
function CreateTab({ onCreated }: { onCreated: () => void }) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setQuestion("")
      setAnswer("")
      // Switch to review tab and reload after 1.5s
...
export default function FlashcardReview() {
  ...
  const handleCreated = useCallback(() => {
    setCards([])
    setLoaded(false)
    setAutoLoad(true)   // trigger auto-load in ReviewTab
    ...
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
      ...
      {tab === "review" ? (
        <ReviewTab
          ...
          setLoaded={setLoaded}
          autoLoad={autoLoad}
          onAutoLoaded={() => setAutoLoad(false)}
        />
      ) : (
        <CreateTab onCreated={handleCreated} />
      )}
    </div>
  )
}