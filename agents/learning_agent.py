...
async def run_learning_agent(message: str, user_id: str) -> AgentResponse:
    ...
    try:
        ...
        if any(k in msg_lower for k in ["create a flashcard", "create flashcard"]):
            q_match = re.search(r'question="([^"]+)"', message)
            a_match = re.search(r'answer="([^"]+)"', message)
            r_match = re.search(r'resource_id=([\w-]+)', message)
        ...
        if any(k in msg_lower for k in ["add ", "reading list", "started reading", "i want to read", "add course", "add book"]):
            title_match = re.search(r"(?:add|read|start)\s+([A-Za-z][^,]+?)(?:\s+by\s+|\s+to\s+|$)", message, re.IGNORECASE)
        ...
        if any(k in msg_lower for k in ["save this note", "take a note", "i learned", "note:", "save note"]):
            ...
            if for_match:
                ...
                note_match = re.search(r"(?:note:|save.*?note:?|i learned[:\s]+)(.*)", message, re.IGNORECASE)
        ...
        if any(k in msg_lower for k in [
            ...
            "show notes", "tool_get_notes", "study notes", "notes for",
        ]):
            resource_match = re.search(r"(?:notes? on|notes? about|notes? for)\s+([A-Za-z][^,\.]+)", message, re.IGNORECASE)
            resource_title = resource_match.group(1).strip() if resource_match else ""
            raw = await tool_get_notes(user_id=user_id, resource_title=resource_title)
        ...
        if any(k in msg_lower for k in ["create a study goal", "study goal", "create goal", "new goal"]):
            ...
            hours_match = re.search(r'weekly target=([\d.]+)', message)
            date_match  = re.search(r'target date=([\d-]+)', message)
