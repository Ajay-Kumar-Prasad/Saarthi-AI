        ...
        try:
            if resource_title:
                rows = await conn.fetch(
                    """
                    SELECT id, entry, logged_at FROM life_logs
                    WHERE user_id = $1 AND domain = 'learning' AND entry ILIKE $2
            ...
            return [
                {"note_id": str(r["id"]), "content": r["entry"], "logged_at": str(r["logged_at"])}
                for r in rows
            ]
        finally:
            await conn.close()
